...
import com.attilaszabo.twentyfivedemo.sources.country.CountryDataSourceImpl
import com.attilaszabo.twentyfivedemo.sources.cutofftime.CutOffTimeDataSource
import com.attilaszabo.twentyfivedemo.sources.cutofftime.CutOffTimeDataSourceImpl
    ...
    fun initialize(applicationContext: Context) {
        startKoin {
            ...
            modules(
                ...
                cutOffTimeModule(applicationContext)
            )
        }
    }
}
...
val CPU_DISPATCHER_QUALIFIER: StringQualifier = named("cpuDispatcher")

val globalDependencies = module {
    single<CoroutineDispatcher>(qualifier = IO_DISPATCHER_QUALIFIER) { Dispatchers.IO }
    single<CoroutineDispatcher>(qualifier = CPU_DISPATCHER_QUALIFIER) { Dispatchers.Default }
    singleOf(::CutOffTimeDataSourceImpl) bind CutOffTimeDataSource::class
    singleOf(::CountryDataSourceImpl) bind CountryDataSource::class
